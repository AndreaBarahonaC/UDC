...
#modelo_modu <- aov(Mod_elas ~ as.numeric(resp[[1]]) * as.character(resp[[2]]) * as.character(resp[[3]]), data = resp)
#nombres_predictores <- c(colnames(prueba1)[2], colnames(prueba1)[3], colnames(prueba1)[1])
##modelo_modu <- aov(Mod_elas ~ resp[[1]] * resp[[2]] * resp[[3]], data = resp)
#names(modelo_modu$terms) <- nombres_predictores
#summary(modelo_modu)
#formula <- as.formula(paste("Mod_elas ~", nombres_predictores[1], "*", nombres_predictores[2], "*", nombres_predictores[3]))
#modelo_modu <- aov(formula, data = resp)
```
...
modelo1<-reactive({
  inFile<-input$file_experimento
  tabla<-carga_experimento_aux(inFile)
  modelo_modu <- aov(Mod_elas ~ `Temperatura` * `No. malla` * `Formulación`, data = tabla)
  imprimir<-summary(modelo_modu)
  imprimir
...
modelo2<-reactive({
  inFile<-input$file_experimento
  tabla<-carga_experimento_aux(inFile)
  modelo_resis <- aov(Resistencia ~ `Temperatura` * `No. malla` * `Formulación`, data = tabla)
  imprimir<-summary(modelo_resis)
  imprimir
...
modelo3<-reactive({
  inFile<-input$file_experimento
  tabla<-carga_experimento_aux(inFile)
  modelo_elon <- aov(Elongacion ~ `Temperatura` * `No. malla` * `Formulación`, data = tabla)
  imprimir<-summary(modelo_elon)
  imprimir
...
modelo4<-reactive({
  inFile<-input$file_experimento
  tabla<-carga_experimento_aux(inFile)
  modelo_tenac <- aov(Tenacidad ~ `Temperatura` * `No. malla` * `Formulación`, data = tabla)
  imprimir<-summary(modelo_tenac)
  imprimir
