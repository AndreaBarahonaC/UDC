...
6. Create a class with a write-only `$password` field. It should have `$check_password(password)` method that returns `TRUE` or `FALSE`, but there should be no way to view the complete password.
...
Bank_writeonly <- R6Class("Bank_writeonly", 
      ...
      public = list(
            ...
            print = function(...){
              cat("Bienvenido al Banco NOSEELNOMBREXD \n")
...
7. Extend the Rando class with another active binding that allows you to access the previous random `value.` Ensure that active binding is the only way to access the value.
